...
const HttpDetailsSchema = Joi.object({
  method: Joi.string().valid('GET', 'PATCH', 'PUT', 'POST', 'DELETE').required()
    .label('Método HTTP')
    .messages({
  ...
  successStatusCode: Joi.number().integer().default(200)
  ...
  pathVariables: Joi.object().pattern(
      Joi.string(),
  ...
  payloadType: Joi.string().valid('file', 'text/plain', 'text/json').optional().allow(null)
    .label('Tipo de Payload')
    .messages({
      'any.only': '{{#label}} deve ser um dos seguintes: {#valids}.',
      'string.base': '{{#label}} deve ser um texto.',
    }),
  responseType: Joi.string().valid('text/plain', 'text/json', 'boolean', 'blank').required()
  ...
  responseMapping: Joi.object().pattern(Joi.string(), Joi.string()).optional().allow(null)
...
const ActionSchema = Joi.object({
  actionType: Joi.string().valid('monitor', 'manage').required()
    .label('Tipo de Ação')
    .messages({
  ...
  protocol: Joi.string().valid('http', 'ssh').required()
    .label('Protocolo')
    .messages({
  ...
  .when(Joi.object({ protocol: Joi.valid('http') }).unknown(), {
    then: Joi.object({
      httpDetails: Joi.required().messages({ 'any.required': 'Detalhes HTTP são obrigatórios quando o protocolo é "http".'}),
      sshDetails: Joi.forbidden().messages({ 'any.forbidden': 'Detalhes SSH não são permitidos quando o protocolo é "http".'})
    })
  })
  .when(Joi.object({ protocol: Joi.valid('ssh') }).unknown(), {
    then: Joi.object({
      sshDetails: Joi.required().messages({ 'any.required': 'Detalhes SSH são obrigatórios quando o protocolo é "ssh".'}),
      httpDetails: Joi.forbidden().messages({ 'any.forbidden': 'Detalhes HTTP não são permitidos quando o protocolo é "ssh".'})
    })
  })
  .label('Ação')
  .messages({
...
export const ProfileActionsSchema = Joi.object().pattern(
...
export const ProfileUpdateSchema = Joi.object({
   ...
   .messages({
     'object.min': 'Pelo menos um campo (nome ou ações) deve ser fornecido para atualização.',
   });
